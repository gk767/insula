...
const COPY = {
  ru: {
    ...
    hoverBody: "Быстрый проход мимо не откроет её — задержите курсор примерно на 0.1 секунды. При медленном приближении остров сначала сжимается снизу, а полностью раскрывается только в верхней трети, у самой камеры.",
    squishWhy: "Капсула лежит поверх строки меню и адресной строки браузера, поэтому может мешать кликам под собой.",
    squishBody: "Если подводить курсор медленно снизу, весь остров сжимается вверх и освобождает место под собой. Если подвести быстро — сжатия не будет.",
  ...
  en: {
    ...
    hoverBody: "A quick pass-by won't open it — pause on it for about 0.1 seconds. Moving in slowly compresses the island first, then opens the full Insula near the top, right by the camera.",
    squishWhy: "The capsule sits on top of your menu bar and address bar, so it can block clicks underneath it.",
    squishBody: "Approach slowly from below and the whole island compresses upward, freeing space underneath. Approach fast and it won't compress.",
  ...
  es: {
    ...
    hoverBody: "Un simple paso rápido no la abre: quédate sobre ella unos 0,1 segundos. Si te acercas despacio, primero la isla se comprime por abajo y luego se abre del todo, cerca de la parte superior, junto a la cámara.",
    squishWhy: "La cápsula queda encima de la barra de menú y de la barra de direcciones, así que puede bloquear los clics de lo que hay debajo.",
    squishBody: "Si te acercas despacio por abajo, toda la isla se comprime hacia arriba y deja sitio debajo. Si llegas rápido, no se comprime.",
  ...
  pt: {
    ...
    hoverBody: "Só passar rápido por cima não abre Insula — fique parado nela por cerca de 0,1 segundo. Chegando devagar, primeiro a ilha se comprime por baixo, e Insula só abre por completo perto do topo, junto à câmera.",
    squishWhy: "A cápsula fica sobre a barra de menu e a barra de endereços, então pode bloquear cliques no que está embaixo dela.",
    squishBody: "Aproxime-se devagar por baixo e a ilha inteira se comprime para cima, liberando espaço embaixo. Se chegar rápido, não comprime.",
  ...
  zh: {
    ...
    hoverBody: "快速经过不会展开它——需要在上面停留大约 0.1 秒。慢慢靠近时，胶囊会先向下压缩，然后才会在最上方、摄像头旁边完全展开。",
    squishWhy: "胶囊盖在菜单栏和地址栏上方，可能会挡住下面的点击。",
    squishBody: "从下方慢慢靠近时，整个胶囊会向上压缩，让出下面的空间。如果很快划过，就不会压缩。",
  ...
  ja: {
    ...
    hoverBody: "サッと通り過ぎただけでは開きません。約0.1秒とどまる必要があります。ゆっくり近づくと、まず島が下から押し込まれ、カメラのすぐそば・上部三分の一の範囲でだけ完全に開きます。",
    squishWhy: "カプセルはメニューバーやアドレスバーの上に重なっているため、下にあるものへのクリックを塞いでしまうことがあります。",
    squishBody: "下からゆっくり近づくと、島全体が上に押し込まれて下にスペースができます。素早く近づくと押し込まれません。",
